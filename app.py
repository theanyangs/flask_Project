...
@app.get('/checkout')
def checkout():
    return render_template('front/checkout.html',products=products, module='checkout')
...
def send_email(to_email, subject, body):
  EMAIL_ADDRESS = 'theanyangs123@gmail.com'  # Your email
  EMAIL_PASSWORD = 'cnwq tfcb fsmr thxv'  # Your email password (or app password)
  ...
  try:
    with smtplib.SMTP_SSL('smtp.gmail.com', 465) as smtp:
      ...
      smtp.send_message(msg)  # <== Add this line to actually send email
...
def place_order(telegram_message=None):
    ...
    for item in cart_items:
      ...
      total = qty * price
      total_usd += total
      order_items_lines.append(f"{title} x {qty} ${price:.2f} = ${total:.2f}")

    order_items_text = "\n".join(order_items_lines)
    total_khr = int(total_usd * 4100)
    ...
    <body>
      ...
      <p><b>Email:</b> {email}</p>
      <p><b>Phone:</b> {phone}</p>
      <p><b>Order Items:</b><br>{order_items_text.replace('\n', '<br>')}</p>
      <p><b>Total:</b> ${total_usd:.2f} (៛{total_khr:,})</p>
    ...
    telegram_message = (
          ...
          f"Phone: {phone}\n"
          f"Order Items:\n{order_items_text}\n"
          f"Total: ${total_usd:.2f} (៛{total_khr:,})"
    )
...
app.config['MAIL_USERNAME'] = 'theanyangs123@gmail.com'        # your gmail address
app.config['MAIL_PASSWORD'] = 'cnwq tfcb fsmr thxv'           # your gmail app password (see note)
app.config['MAIL_DEFAULT_SENDER'] = ('theanyang', 'theanyangs123@gmail.com')
...
def contact():
    ...
    try:
        msg = Message(subject=f"Contact Form Message from {first_name} {last_name}",
                      recipients=['theanyangs123@gmail.com'],  # your email to receive the messages
                      body=email_body)
        mail.send(msg)
        return jsonify({'success': 'Message sent successfully!'}), 200  # <-- fixed here
    except Exception as e:
